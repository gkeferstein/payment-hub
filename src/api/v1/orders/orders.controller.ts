...
export class OrdersController {
   ...
   * Get order by ID
   * Query params:
   *  - include=payments (optional): Include payment information
   */
  async getOrderById(req: Request, res: Response): Promise<void> {
    try {
      ...
      let order;
      if (includePayments) {
        order = await orderService.getOrderWithPayments(req.params.id);
      } else {
        order = await orderService.getOrderByIdWithItems(req.params.id);
      }
